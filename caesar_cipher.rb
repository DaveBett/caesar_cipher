...
def caesar_cipher(string, modifier)
  # takes in a string and the shift factor and then outputs the modified
  # string using a right shift
  alphabet = %w[a b c d e f g h i j k l m n o
  ...
  (0...string.length).each do |i|
    ...
    alphabet.each_with_index do |letter, index|
      if alphabet.include?(string_letter) == false
        # checks if the current character is not in the alphabet array,
        # this includes capital letters

        if letter.upcase == string_letter && alphabet.include?(string_letter.downcase)
          # checks if the current character is capitalized and adds it
          # to the cryptid array if it is
          cryptid[i] = alphabet[index + mod].upcase
          break
        elsif !alphabet.include?(string_letter.downcase)
          # checks if the current character is not a letter and adds it
          # to the cryptid array if it is
        ...
        # checks if the current character is a letter and adds it
        # to the cryptid array if it is
